...
def build_peak_metrics_tables(timestamps, baseline_grid_import, optimized_grid_import, top_n=10):
    ...
    top_peak_intervals = (
        peak_df.sort_values("peak_reduction", ascending=False)
        .head(top_n)
        .copy()
    )
...
def build_battery_utilization_table(solution_summary, battery_soc, battery_charge_power, battery_discharge_power):
    ...
    summary_rows = [
        ...
        {"Metric": "Maximum charge power used", "Value": float(battery_charge_power.max()) if battery_charge_power.size else np.nan, "Unit": "kW"},
        {"Metric": "Maximum discharge power used", "Value": float(battery_discharge_power.max()) if battery_discharge_power.size else np.nan, "Unit": "kW"},
    ]
