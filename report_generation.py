...
def generate_pdf_report(run_dir: Path, solution_summary: dict, settings_snapshot: dict, input_dict: dict) -> Path:
    ...
    with PdfPages(report_path) as pdf:
        ...
        assumptions_lines = [
            ...
            f"- Import price type: {'Constant' if use_constant_import_price else 'Dynamic (ENTSO-E)'}",
            f"- Import price value: {_fmt_num(val_import) if use_constant_import_price else 'Variably calculated'} [CHF/kWh]",
            f"- Export price type: {'Constant' if use_constant_export_price else 'Dynamic (ENTSO-E)'}",
            f"- Export price value: {_fmt_num(val_export) if use_constant_export_price else 'Variably calculated'} [CHF/kWh]",
